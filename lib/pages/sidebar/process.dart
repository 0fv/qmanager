...
class Process extends StatelessWidget {
  ...
  Future<dynamic> _getListData() async {
    return await processApi.getData();
  }
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          FlatButton(
              child: Text("立即结束调查"),
              onPressed: () async {
                bool v = await alertDialog("确定立即结束？", context);
                if (v == true) {
                  try {
                    await processApi.updateToFinish(row["id"]);
  ...
  DataCell _getDataCell(int index, var value) {
    if (index == 3) {
      return DataCellUtil.getDataCell(value, 200,
          setText: (value) => DateTime.now().isAfter(DateTime.parse(value))
              ? Text(
                  "$value",
